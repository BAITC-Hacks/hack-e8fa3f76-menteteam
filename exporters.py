...
def export_pdf(result: MeetingResult) -> bytes:
    ...
    font_path = next((path for path in font_paths if __import__("os").path.exists(path)), None)
    if font_path:
    ...
    story = [Paragraph(f"Протокол: {escape(result.title)}", styles["Title"]), Spacer(1, 10),
             Paragraph("Саммари", styles["Heading2"]), Paragraph(escape(result.summary or "Не сформировано"), styles["Normal"]),
             Paragraph("Решения", styles["Heading2"])]
    for d in result.decisions: story.append(Paragraph(f"• {escape(d)}", styles["Normal"]))
    ...
    for s in result.transcript: story.append(Paragraph(f"[{s.start:.1f}–{s.end:.1f}] {escape(s.speaker)}: {escape(s.text)}", styles["Normal"]))
    if result.questions:
        story.append(Paragraph("Нужно уточнить", styles["Heading2"]))
        for q in result.questions: story.append(Paragraph(f"• {escape(q)}", styles["Normal"]))
